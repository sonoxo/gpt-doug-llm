...
async def health():
    async with httpx.AsyncClient(timeout=4) as client:
        async def probe(url):
            try: return (await client.get(url)).status_code < 500
            except httpx.HTTPError: return False
        gateway=await probe(f"{LITELLM}/health/liveliness")
        search=await probe(f"{SEARXNG}/")
    return {"control":True,"gateway":gateway,"search":search,"ready":gateway and search}

@app.get("/api/search")
async def research(q:str=Query(min_length=2,max_length=300)):
    async with httpx.AsyncClient(timeout=20) as client:
        try: response=await client.get(f"{SEARXNG}/search",params={"q":q,"format":"json","safesearch":1})
        except httpx.HTTPError as exc: raise HTTPException(502,"Research service unavailable") from exc
    if response.status_code != 200: raise HTTPException(502,"Research provider rejected the request")
    rows=response.json().get("results",[])[:10]
    return {"query":q,"results":[{"title":r.get("title"),"url":r.get("url"),"content":r.get("content"),"engine":r.get("engine")} for r in rows]}
