...
def workflow_names() -> list[str]:
    ...
    if not p.exists():
        return []
    return sorted(x.name for x in p.glob("*.yml")) + sorted(
        x.name for x in p.glob("*.yaml")
    )
...
def write_ecosystem() -> None:
    cards = [
        (
            ...
            "#A970FF",
            exists_any("zyra_chat.py", "*doug*.py"),
        ),
        (
            "Wakeup3lm",
            "IDE-native LLM execution kernel",
            "#3DE1FF",
            exists_any("wakeup3lm/runtime.py"),
        ),
        (
            "Ontology",
            f"typed graph • {ontology_count()} core object types",
            "#7CFF6B",
            exists_any("wakeup3lm/ontology.py", "safety-shield/ontology/*"),
        ),
        (
            "Palantir AIP / Foundry",
            "model + Logic + authorized Ontology",
            "#FFB454",
            exists_any("palantir*aip*.py", "palantir_stack.py", "palantir_foundry.py"),
        ),
        (
            "Agent + Tool Runtime",
            "files • terminal • build • validation",
            "#FF6BD6",
            exists_any("agents/**", "wakeup3lm/workspace.py"),
        ),
        (
            "Security Gates",
            f"{len(security_workflows())} repo security/control workflows",
            "#FF5D73",
            len(security_workflows()) > 0,
        ),
        (
            "Evidence",
            f"{test_count()} Python test modules + audit state",
            "#FFD166",
            test_count() > 0,
        ),
        (
            "Ship",
            "preview • release • deployment adapters",
            "#2DD4BF",
            exists_any(".github/workflows/docker-publish.yml", "*apollo*.py"),
        ),
    ]
...
def write_agent_loop() -> None:
    steps = [
        ("1", "ASK", "Describe the app or change"),
        ("2", "PLAN", "Wakeup3lm structures the mission"),
        ("3", "GROUND", "Ontology + authorized AIP context"),
        ("4", "ACT", "Allowlisted tools edit/run/build"),
        ("5", "VERIFY", "Tests + security gates + preview"),
        ("6", "REPAIR", "Failures feed the next decision"),
        ("7", "CHECKPOINT", "Keep evidence + rollback point"),
        ("8", "SHIP", "Publish only through approved path"),
    ]
    ...
    for x in (340, 660, 980):
        out.append(arrow(x, 209, x + 50, 209))
    for x in (980, 660, 340):
        out.append(arrow(x + 50, 389, x, 389))
