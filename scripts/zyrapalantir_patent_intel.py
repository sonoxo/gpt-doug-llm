...
This tool reads the repository's provenance-preserving public patent metadata.
It performs no network calls and makes no legal conclusions.
"""
...
def print_records(source: dict, query: str | None = None) -> int:
    ...
    for record in matched:
        print(f"{record.get('document_number')}  {record.get('publication_date')}")
        print(f"  {record.get('title')}")
        print(f"  themes: {', '.join(record.get('themes', []))}")
    if not matched:
        print("No selected records matched. This local index contains a curated subset of the supplied USPTO results, not the full result set.")
...
def doctor() -> int:
    ...
    print(f"   selected ...... {len(document_numbers)} records")
