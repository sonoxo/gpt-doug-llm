...
class PalantirCprProbeTests(unittest.TestCase):
    def make_repo(self, root: Path) -> None:
        files = {
            ...
            "safety-shield/ontology/palantir-maven-glass-onion.json": json.dumps(
                {
                    "ontology": "PalantirMavenGlassOnion",
                    "guardrails": {
                        "automaticPublishing": False,
                        "storesCredentialsInGit": False,
                    },
                }
            ),
            "safety-shield/ontology/zyrapalantir-live.json": json.dumps(
                {
                    "ontology": "ZYRAPALANTIRLive",
                    "mode": "DEFENSIVE_MONITORING_ONLY",
                    "controls": {
                        "automaticExternalAction": False,
                        "artifactExecutionFromMonitor": False,
                        "humanApprovalRequiredForExternalAction": True,
                    },
                }
            ),
            "redpanda-desktop/palantir-maven": "#!/bin/sh\nexit 0\n",
            "scripts/zyrapalantir": "#!/bin/sh\nexit 0\n",
            "scripts/zyrapalantir_live.py": "VALUE = 1\n",
        }
    ...
    def test_probe_passes_valid_local_integration(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(result["checks"]["palantir_maven_glass_onion"])
            self.assertTrue(result["checks"]["zyrapalantir_live_policy"])
            self.assertEqual(
                result["checks"]["sovereignty_runtime"]["preferred_accelerator"],
                "cpu",
            )
