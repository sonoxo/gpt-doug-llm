...
DENY_PATTERNS = [
    ...
    # --- PATCHED: disk destruction commands (from core zyra.py) ---
    r"\bmkfs\b.*/dev/",               # mkfs.ext4 /dev/sda1, mkfs /dev/sdb
    r"\bd\s*d\s+if=.*\s+of=/dev/",         # dd if=/dev/zero of=/dev/sda
    r"\bdiskutil\s+erase\b",              # diskutil erase (macOS)
    # --- PATCHED: prompt injection (from core zyra.py) ---
    r"(?i)\b(?:ignore|disregard|forget|discard)\s+(?:the\s+)?(?:all\s+|any\s+)?(?:previous|prior|system|earlier|above)(?:\s+\w+)?\s+instructions",
    # --- PATCHED: security bypass requests (from core zyra.py) ---
    r"(?i)\b(?:disable|bypass|evade|circumvent|defeat|turn\s+off|shut\s+off|get\s+around|work\s+around)\b.{0,35}\b(?:security|authentication|auth|firewall|guardrail|safety\s+check|zyra)\b",
]
...
DESTRUCTIVE_ACTION_WORDS = [
    ...
    "reformat", "format disk", "overwrite disk",
]
BROAD_TARGET_WORDS = [
...
def _strip_invisible(text: str) -> str:
    # Remove all invisible characters entirely. This can fuse words that
    # were separated only by zero-width chars ("r\u200bm" → "rm"), which is
    # exactly what we want for pattern matching against known attack patterns.
    # The semantic matcher also handles the space-separated variant via
    # the dual-normalization approach in review() below.
    return _INVISIBLE_RE.sub("", text)

def _strip_invisible_to_space(text: str) -> str:
    """Alternative normalization that converts zero-width chars to spaces
    instead of removing them. Used as a second pass so semantic matching
    can still find multi-word phrases like 'root directory' that were
    separated by invisible characters."""
...
def review(task: dict) -> tuple[bool, str]:
    ...
    normalized_spaced = _strip_invisible_to_space(_normalize_confusables(prompt))

    # Run checks against the raw prompt, the stripped (ZWS removed) variant,
    # and the spaced (ZWS → space) variant. Each catches different evasions:
    #   - stripped:  catches "r\u200bm" → "rm" (fused token matches rm pattern)
    #   - spaced:    catches "root\u200bdirectory" → "root directory" (semantic)
    for check in (_pattern_hit, _semantic_hit, _decoded_hit):
        for variant, variant_label in ((prompt, None), (normalized, "confusable-normalized"), (normalized_spaced, "confusable-normalized")):
            reason = check(variant)
            if reason:
                if variant_label:
                    reason = f"{variant_label} payload: {reason}"
                level = classify(prompt, False, reason, [])
                _log(task_id, "BLOCK", reason, level)
                return False, reason
